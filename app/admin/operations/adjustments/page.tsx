...
import { CheckCircle, XCircle, ArrowRightLeft, Clock, Calendar, User, Settings, AlertTriangle, History } from 'lucide-react'
import { taskChangeLogs, tasks, type TaskChangeLog } from '@/lib/mock-data'
...
export default function TaskSchedulingPage() {
  ...
  const pendingCount = taskChangeLogs.filter((l) => l.status === 'pending').length
  const approvedCount = taskChangeLogs.filter((l) => l.status === 'approved').length
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="flex items-center justify-between p-4">
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold">{new Set(taskChangeLogs.map((l) => l.taskId)).size}</p>
            </div>
        ...
        <Card>
          <CardContent className="flex items-center justify-between p-4">
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">停课/补课</p>
              <p className="text-2xl font-bold">{taskChangeLogs.filter((l) => l.changeType === 'cancel' || l.changeType === 'makeup').length}</p>
            </div>
                  ...
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        {log.status === 'pending' && (
                          <>
                            <Button variant="ghost" size="icon" className="text-green-600 h-8 w-8" onClick={() => toast.success('审批通过')}><CheckCircle className="h-4 w-4" /></Button>
                            <Button variant="ghost" size="icon" className="text-red-600 h-8 w-8" onClick={() => toast.error('已驳回')}><XCircle className="h-4 w-4" /></Button>
                          </>
                        )}
                      </div>
                    </TableCell>
