...
export default function CourseLaunchPage() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((r) => r.launchStatus === activeTab)
  }, [activeTab, records])

  const stats = useMemo(() => {
    const total = records.length
    const ready = records.filter((r) => r.launchStatus === 'ready').length
    const launched = records.filter((r) => r.launchStatus === 'launched').length
    const ended = records.filter((r) => r.launchStatus === 'ended').length
        ...
        <Card className="bg-blue-50/50">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">本周开课</span>
            </div>
          ...
          <Table>
            ...
            <TableBody>
              {filteredRecords.map((record) => {
                const status = statusConfig[record.launchStatus]
                const StatusIcon = status.icon
                const launchCheck = selectedRecord?.id === record.id ? computeLaunchChecklist(selectedRecord) : computeLaunchChecklist(record)
                const allReady = launchCheck.every((c) => c.passed)
                return (
                  <TableRow key={record.id}>
                    ...
                    <TableCell>
                      <span className="text-xs text-muted-foreground">
                        周{['日', '一', '二', '三', '四', '五', '六'][record.dayOfWeek]}
                        第{record.periods.join('-')}节
                      </span>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" className={cn('text-xs',
                        record.prepStatus === 'completed' ? 'text-emerald-600' : 'text-amber-600'
                      )}>
                        {record.prepStatus === 'completed' ? '已完成' : record.prepStatus === 'in_progress' ? '备课中' : '待备课'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => router.push(`/admin/operations/course-launch/${record.id}`)}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        {record.launchStatus === 'ready' && (
                          <Button
                            size="sm"
                            onClick={() => openLaunchDialog(record)}
